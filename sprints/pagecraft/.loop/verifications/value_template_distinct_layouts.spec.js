...
test('SaaS, Event, and Portfolio templates render visually distinct layouts', async ({ page }) => {
...
test('template-specific CSS classes apply distinct styling', async ({ page }) => {
  await page.goto('/');
  await page.waitForSelector('#template-cards .template-card');
...
test('hero section layout varies between templates', async ({ page }) => {
  const heroLayouts = [];
